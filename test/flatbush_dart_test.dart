...
void main() {
  group('Flatbush', () {
    ...
    Flatbush createIndex() {
      final index = Flatbush(numItems: data.length ~/ 4);
    ...
    Flatbush createSmallIndex(int numItems, int nodeSize) {
      final index = Flatbush(numItems: numItems, nodeSize: nodeSize);
    ...
    test('does not freeze on numItems = 0', () {
      expect(() => Flatbush(numItems: 0), throwsA(isA<AssertionError>()));
    });
    ...
    test('returns index of newly-added rectangle', () {
      const count = 5;
      final index = Flatbush(numItems: count);
